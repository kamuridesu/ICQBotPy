...
async def getBotInfo(
    session: aiohttp.ClientSession, token: str, endpoint: str
) -> dict[str, typing.Any]:
    response: Response = await sendGetRequest(
        session, endpoint + "/self/get?token=" + token
    )
...
async def uploadFile(
    ...
    route: str,
    query: str,
    file_id: typing.Union[str, None] = None,
    ...
    if file_id:
        ...
        response = await sendPostRequest(
            session, endpoint + route + query, files=content
        )
...
async def getFileInfo(
    session: aiohttp.ClientSession, token: str, endpoint: str, file_id: str
) -> dict[str, typing.Any]:
    route = "/files/getInfo?"
