...
try:
    from exceptions.ClientErrors import ClientError, InvalidTokenError
...
class RawApiMapper:
    ...
    def getSelfInfo(self) -> dict[str, typing.Any]:
        response: requests.Response = fetcher("get", self.endpoint + "/self/get?token=" + self.token)
    ...
    def uploadFile(self, route: str, query: str, file_id: typing.Union[str, None]=None, file: typing.Union[str, bytes, None]=None) -> requests.Response:
        response: typing.Union[None, requests.Response] = None
        if file_id:
            query += f"&fileId={file_id}"
            response = fetcher("get", self.endpoint + route + query)
        elif file:
            ...
            response = fetcher("post", self.endpoint + route + query, files=content)
